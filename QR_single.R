# this script perform quantile regression on single variables
...
one_pixel = lapply(files, function(f) {
    ...
    read.table(path_f, header=FALSE, skip=1)
})
...
# generate subsamples for plot
xx = c(seq(1,365, 5))
yy = c(seq(1,upper_year,1))
Xidx = c(outer(xx, (yy-1) * 365, "+"))
Xsubsample = X[Xidx,] 
...
matplot(t(pt1-273.15), type='p',pch=1,col='gray', ylim=c(-20,35), ylab='temperature (Deg C)',
        xlab = 'Day of the year')
par(new=TRUE)
matplot(z[1:365,]-273.15,type = 'l',pch=1:5,col = 1:5, ylim=c(-20,35), ylab='temperature (Deg C)',
        xlab = 'Day of the year') #plot
legend(200,10, legend = qq, col=1:5, pch=1) # optional legend)
title('quantile fit of daily maximum temperature at NYC in 1990')

# plot quantile coefficient in the last year


matplot(t(pt2-273.15), type='p',pch=1,col='gray', ylim=c(-10,40), ylab='temperature (Deg C)',
        xlab = 'Day of the year')
par(new=TRUE)
matplot(z[(13140-365+1):13140,]-273.15,type='l',pch=1:5,col = 1:5, ylim=c(-10,40),ylab='temperature (Deg C)',
        xlab = 'Day of the year') #plot

legend(200,18, legend = qq, col=1:5, pch=1) # optional legend)
title('quantile fit of daily maximum temperature at NYC in 2080')
